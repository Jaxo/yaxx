...
#if defined _WIN32
#pragma warning(disable:4307)     // multiply converted...
#pragma warning(disable:4308)     // negative integral..
#endif
...
/* sbrk for palm OS */
static int psb_maxPtrs = 0;
static void * psb_ptrs[100] = {0};
static int psb_maxSize = 16 * 4096;    // ie: 64K
static void * psb_sbrkTop = 0;

static void * palmOS_sbrk (long size)
{
   // try allocating the maximum size
   if (size > 0) {
      void * ptr = 0;
      for (size = psb_maxSize; size; size >>= 1) {
         ptr = MemPtrNew(size);
         if (ptr) {                // found it!
            psb_maxSize = size;
            do {                   // step by page size
               MemPtrFree(ptr);    // try to get more...
               size += malloc_getpagesize;
               ptr = MemPtrNew(size);
            }while (ptr);
            size -= malloc_getpagesize;
            ptr = MemPtrNew(size);
            assert (ptr);          // the biggest chunk
            break;
         }
      }
      if (!ptr) {
      ...
      // save ptrs so they can be freed during cleanup
      psb_ptrs[psb_maxPtrs++] = ptr;
      psb_sbrkTop = (char *)ptr + size;
      // round, as required by the specs
      return (void *)(
         (((CHUNK_SIZE_T) ptr) + malloc_getpagesize-1) & ~(malloc_getpagesize-1)
      );
   }else if (size < 0) {
      // no trim support
      return (void *)MORECORE_FAILURE;
...
// cleanup any allocated MemPtr
// called as last thing before shutting down interpreter
void dlmalloc_palmos_cleanup()
{
   void ** pptr;
   for (pptr = psb_ptrs; pptr < psb_ptrs + psb_maxPtrs; ++pptr) {
      MemPtrFree(*pptr);
   }
}
